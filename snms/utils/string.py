...
def to_unicode(text):
    ...
    return fix_broken_string(text, as_unicode=True) if isinstance(text, str) else str(text)


def fix_broken_obj(obj):
    if isinstance(obj, dict):
        return dict((k, fix_broken_obj(v)) for k, v in obj.items())
...
def camelize(name):
    ...
    return underscore + ''.join(x.title() for x in parts)
...
def crc32(data):
    ...
    When a str object is passed, it is encoded as UTF-8.
    """
...
def inject_unicode_debug(s, level=1):
    ...
    """
    try:
        unicode_debug_level = int(os.environ.get('SNMS_UNICODE_DEBUG', '0'))
    except ValueError:
