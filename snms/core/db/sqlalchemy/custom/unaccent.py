...
from snms.utils.string import to_unicode
...
SQL_FUNCTION_UNACCENT = '''
    CREATE FUNCTION snms.indico_unaccent(value TEXT)
        RETURNS TEXT
    AS $$
    BEGIN
...
def define_unaccented_lowercase_index(column):
    ...
    use of this index, use this criterion when querying the table::
        ...
        db.func.snms.indico_unaccent(db.func.lower(column)).ilike(...)
    ...
    def _after_create(target, conn, **kw):
        ...
        col_func = func.snms.indico_unaccent(func.lower(column))
...
def unaccent_match(column, value, exact):
    ...
    return db.func.snms.indico_unaccent(db.func.lower(column)).ilike(db.func.snms.indico_unaccent(value))
