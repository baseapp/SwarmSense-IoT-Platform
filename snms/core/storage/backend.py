...
def get_storage(backend_name):
    ...
    snms config.  Once a backend has been used it is assumed to
...
class Storage(object):
    ...
    def get_local_path(self, file_id):
        ...
        with self.open(file_id) as fd:
            with NamedTemporaryFile(suffix='snms.tmp', dir=config.TEMP_DIR) as tmpfile:
...
class FileSystemStorage(Storage):
    ...
    def open(self, file_id):
        try:
            ...
            raise StorageError('Could not open "{}": {}'.format(file_id, e)).with_traceback(sys.exc_info()[2])
    ...
    def save(self, name, content_type, filename, fileobj):
        try:
            ...
            raise StorageError('Could not save "{}": {}'.format(name, e)).with_traceback(sys.exc_info()[2])

    def delete(self, file_id):
        try:
            os.remove(self._resolve_path(file_id))
        except Exception as e:
            raise StorageError('Could not delete "{}": {}'.format(file_id, e)).with_traceback(sys.exc_info()[2])

    def getsize(self, file_id):
        try:
            return os.path.getsize(self._resolve_path(file_id))
        except Exception as e:
            raise StorageError('Could not get size of "{}": {}'.format(file_id, e)).with_traceback(
                sys.exc_info()[2])
    ...
    def send_file(self, file_id, content_type, filename, inline=True):
        try:
            ...
            raise StorageError('Could not send "{}": {}'.format(file_id, e)).with_traceback(sys.exc_info()[2])
